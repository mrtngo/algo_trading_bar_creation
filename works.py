...
# max_trades = st.sidebar.number_input("Max Trades to Fetch (0 for no limit)", value=50000, min_value=0, step=1000)

# # Fetch BTC/USDT aggTrades with feedback
...
#             data = resp.json()
#             if not data or ('code' in data and data['code'] != 200):
#                 st.warning("API returned no data or an error. Stopping fetch.")
#                 break
#             all_trades.extend(data)
#             records += len(data)
#             progress = min(records / max_records, 1.0) if max_records else min((params['startTime'] - start_ts) / (end_ts - start_ts), 1.0)
#             pbar.progress(progress)
#             last_ts = data[-1]['T']
#             if len(data) < 1000 or last_ts >= end_ts or (max_records and records >= max_records):
#                 break
#             params['startTime'] = last_ts + 1
#             t.sleep(0.5)  # Increased to avoid rate limits
...
#     df['side'] = np.where(df['m'], -1, 1)  # True if buyer is market maker (sell), else buy
...
#     trades = fetch_trades("BTCUSDT", lookback_days, max_trades if max_trades > 0 else 0)
...
# def run_model(bars):
#     if len(bars) < 10:  # Need enough data for meaningful model
#         return np.nan
#     df = bars.copy()
...
#     df = df.dropna()
#     if len(df) < 10:  # Ensure enough data after dropna
#         return np.nan
...
# Fetch trades
st.header("₿ Advanced BTC Trading Bars Analysis")

col1, col2 = st.columns([3, 1])
with col1:
    st.write("Enhanced version with better rate limiting and larger data fetching capabilities")
with col2:
    ...
    if use_async:
        ...
        try:
            ...
            trades_data = loop.run_until_complete(
                async_fetch_wrapper("BTCUSDT", lookback_days, max_trades if max_trades > 0 else 0, delay_ms, batch_size)
            )
            ...
            trades = cached_fetch_trades("BTCUSDT", lookback_days, max_trades if max_trades > 0 else 0, delay_ms, batch_size) if use_cache else fetch_trades_sync_chunked("BTCUSDT", lookback_days, max_trades if max_trades > 0 else 0, delay_ms, batch_size)
    else:
...
if not trades.empty:
    ...
    with col3:
        st.metric("Avg Trade Size", f"{trades['qty'].mean():.4f} BTC")
    with col4:
        st.metric("Total Volume", f"{trades['qty'].sum():.1f} BTC")
    
    st.success(f"Successfully fetched {len(trades):,} trades from {start_time} to {end_time}")
else:
    st.error("No trades were fetched.")
    st.stop()
    ...
    with st.expander(f"{name} Bars Chart", expanded=True):
        ...
        try:
            fig, ax = mpf.plot(
                ...
                volume=True, 
                returnfig=True,
                title=f"{name} Bars ({len(bars)} bars)",
                style='yahoo',
                figsize=(12, 8)
            )
            st.pyplot(fig)
